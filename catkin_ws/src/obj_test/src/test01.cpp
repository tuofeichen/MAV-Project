...
// Initialize variables
	...
	ros::Publisher wall_pub = nh.advertise<geometry_msgs::Point>("PerpendicularWallFind", 1000);
	...
	//TEMPLATE = imread( "/home/felipe/catkin_ws/src/obj_test/witcher_cover.jpg", IMREAD_GRAYSCALE );
	
	
	TEMPLATE = imread( "/home/odroid/MAV-Project/px4_ws/src/obj_test/book.jpg", IMREAD_GRAYSCALE );
...
void CheckForWall(cv::Mat Depth)
{
	...
	int l_counter=0;
	float alpha, betha;
	float angle_per_pixel_rad = (58./320.)*PI/180.;
	...
	if(valid_position && fabs(r-l)/max(r,l) < 0.2 ) 
	{
		...
		if(l > r)
		{
 	 		 alpha = asin(r/D*sin(FOV));
   			 angle.x = alpha + FOV/2 - PI/2;
		}
		else
		{
    		betha = asin(l/D*sin(FOV));
    		angle.x = PI/2 - FOV/2 - betha;
		}
    
		if(l < 3000 && r < 3000 && r!=0 && l!=0) 
		{
		
			angle.y = angle.x*180/PI; // in degrees
		}
		else
		{
			angle.x = -5000;
	    ...
	    //cout << "angle(in degrees): " << angle.y << endl;
		//cout << "angle(in rad): " << angle.x << endl;
	}
	else
	{
		angle.x = -5000;

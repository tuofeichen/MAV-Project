...
int Frontend::getNewNode(Frame& frame, Eigen::Isometry3d& tm, Eigen::Matrix<double, 6, 6>& im)
{
	if( queueSRgb.size() > 1 && queueSDepth.size() > 1 &&
		queueSTimeStamp.size() > 1 && queueSTm.size() > 1 &&
		queueSIm.size() > 1 && queueToNode.size() > 1 )
	{
		// create images
		...
		deserializeTimeStamp(queueSTimeStamp.front(), *timeStamp);
		deserializeTm(queueSTm.front(),trafoMat);
		deserializeIm(queueSIm.front(),infoMat);
		int toNode = queueToNode.front();

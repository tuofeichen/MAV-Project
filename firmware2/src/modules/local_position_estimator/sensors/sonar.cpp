...
void BlockLocalPositionEstimator::sonarCorrect()
{
	...
	// residual
	Vector<float, n_y_sonar> r = y - C * _x; // measurement after noise term
	...
	if (_sonarFault < fault_lvl_disable) {
		Matrix<float, n_x, n_y_sonar> K =
			_P * C.transpose() * S_I; //here C matrix can clear up nonrelevant entries

		Vector<float, n_x> dx = K * r; // r = y - C * _x
		// dx = K * (y - C*_x)

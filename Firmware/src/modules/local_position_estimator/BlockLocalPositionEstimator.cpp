...
void BlockLocalPositionEstimator::update()
{
	...
	if (!_lastArmedState && armedState) {
		// reinitialize covariance as well. 
		initP();
		...
		// reset flow integral and sensor update flag
		_flow_cnt = 0;
		_baro_cnt = 0;
		_sonar_cnt = 0;
	...
	if (_baroAltHome<10)
	{
		mavlink_and_console_log_info(&mavlink_log_pub, "[LPE] baro alt not initialized");
	}
	// determine if we should start estimating
	_canEstimateZ =
		(_baroInitialized && _baroFault < fault_lvl_disable);

	_canEstimateXY =(_flowInitialized && _flowFault < fault_lvl_disable) ||
	...
	_canEstimateT =	(_sonarInitialized && _sonarFault < fault_lvl_disable)||
	...
	if (flowUpdated) {
		...
		_flow_cnt = 0;
		// mavlink_and_console_log_info(&mavlink_log_pub,"Flow udpated	!");
		if (!_flowInitialized) {
			flowInit();
		} else {
...
{	
	// mavlink_and_console_log_info(&mavlink_log_pub,"Alt mode is %d", _alt_mode.get());
	...
	if (PX4_ISFINITE(_x(X_x)) && PX4_ISFINITE(_x(X_y)) && PX4_ISFINITE(_x(X_z)) &&
		...
		_pub_lpos.get().z =  (_alt_mode > 0)? (-agl()):(_x(X_z));	//use agl() or X(z) depend on parameter setting
		...
		_pub_lpos.get().ref_lat = _sonarFault;//_map_ref.lat_rad * 180 / M_PI;
		_pub_lpos.get().ref_lon = _baroFault;//_map_ref.lon_rad * 180 / M_PI;
...
void BlockLocalPositionEstimator::publishEstimatorStatus()
{
	if (PX4_ISFINITE(_x(X_x)) &&
		...
		_pub_est_status.get().states[10] = _flowX;
	    _pub_est_status.get().states[11] = _flowY;

	    _pub_est_status.get().states[12] =  (_flow_cnt)%100; //lat;
		_pub_est_status.get().states[13] =  (_sonar_cnt)%100; //lat;
		
		_pub_est_status.get().covariances[10] =  (!_sonarInitialized); // _flowX;
		_pub_est_status.get().covariances[11] =  (!_flowInitialized);  // _flowY;//predicted; // indicate whether prediction is persued

		_pub_est_status.get().n_states = _baro_cnt;
		_pub_est_status.get().nan_flags = _baroAltHome;
...
void BlockLocalPositionEstimator::publishGlobalPos()
{
	...
	if (PX4_ISFINITE(lat) && PX4_ISFINITE(lon) && PX4_ISFINITE(alt) &&
		...
		_pub_gpos.get().lat =  lat; // (_sonar_cnt)%100; //lat;
		_pub_gpos.get().lon =  lon; // (_flow_cnt)%100 ;
		_pub_gpos.get().alt =  alt;
		_pub_gpos.get().vel_n = _x(X_vx); //track updates
...
bool BlockLocalPositionEstimator::predict()
{
	// if can't update anything, don't propagate
	// state or covariance
	if (!_canEstimateXY && !_canEstimateZ) { return 0; }
	...
	if (_integrate.get() && _sub_att.get().R_valid) {
		...
		mavlink_and_console_log_info(&mavlink_log_pub,"Error R!");
		_u = Vector3f(0, 0, 0);
	}
	...
	_P += (A * _P + _P * A.transpose() +
	       B * R * B.transpose() + Q) * getDt(); // B* R *B is the control input noise (embed into the process noise)
	return 1;
}

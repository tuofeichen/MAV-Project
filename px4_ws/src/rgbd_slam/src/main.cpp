...
// static bool processLPE(Matrix4f prev_pos, Matrix4f pos, Matrix4f& tmCurToNode, Matrix<float, 6, 6>& imCurToNode)
// {
// 	tmCurToNode = 
// }
...
static void deleteLog()
{
	...
	for (int i = 0; i< 100; i++)
	{
		sprintf(imgName, "/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/%d_new_node_rgb.png",i);
		std::remove(imgName);
		sprintf(imgName, "/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/%d_new_node_dep.png",i);
		std::remove(imgName);
	}
...
{	
	...
	static std::ofstream logPos, logTm;
...
//	AsusProLive::start();
	AsusProLiveOpenNI2::start();
	logPos.open("/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/frontend.csv",std::ofstream::out | std::ofstream::trunc);
    logPos << "X,Y,Z,LPE_Z"<<endl;

    logTm.open("/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/trans.txt",std::ofstream::out | std::ofstream::trunc);
    logTm << "Rx,Ry,Rz,T"<<endl;
   ...
   // grab some images until brightness is adjusted correctly
	...
	while(backend.running() && AsusProLiveOpenNI2::running())
	{
		...
		backend.setNewNode(frames[state],tmCurToNode,imCurToNode,1); // first node 

		// save first node 
		sprintf(imgName, "/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/1_new_node_rgb.png");
		cv::imwrite(imgName,frames[state].getGray());
		sprintf(imgName, "/home/odroid/MAV-Project/px4_ws/src/rgbd_slam/Frames/1_new_node_dep.png");
	...
	while(backend.running() && AsusProLiveOpenNI2::running() && ros::ok())
	{
		...
		if(processFrame(frames[state],frames[state?0:1],tmCurToNode,imCurToNode,newNode))
		{
			...
			// cameraPos = backend.getCurrentPosition(); // quite irrelevant to frontend
		...
		if (newNode) // spit out processing time if needed

	...
	while(ros::ok())
	{
		...
		if(take_off_done)
		{
			...
			else
			{
				...
				if(depth_central_pixel - 10 > 0 && !turning)
				{
				...
				else
				{
					...
					else
					{
						...
						if((3.14 + initial_yaw - pos_read_.yaw) > 3.14/1000. && !go_foward)	
						{	
							ROS_INFO("Turning");
							turning = true;
						...
						else
						{
							...
							if(depth_central_pixel - 1000 > 10)
							{
								cmd.distance = (depth_central_pixel - 1000);
								cmd.code = 0;
								cmd_pub.publish(cmd);
							}
							else
							{
								ROS_INFO("You have reached the 2nd wall");
								return 0;

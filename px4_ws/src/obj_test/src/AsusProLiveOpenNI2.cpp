...
bool AsusProLiveOpenNI2::grab(Frame& frame)
{
	...
	if(colorReadyFlag && depthReadyFlag)
	{
		...
		boost::mutex::scoped_lock(grabbingMutex);
		cv::cvtColor(colorImg,rgb ,CV_BGR2RGB);
		cv::cvtColor(colorImg,gray,CV_BGR2GRAY);
		// cout << "copy rgb start" << endl; 
		rgb.copyTo(*rgb_ptr);
		// cout << "copy gray start" << endl;
		gray.copyTo(*gray_ptr);
		// cout << "copy depth start" << endl;
		depthImg.copyTo(*dep_ptr);
		// cout << "copy time start" << endl;
		*time_ptr = depthTimeStamp; // colorTimeStamp
		
		frame = Frame(rgb_ptr,gray_ptr,dep_ptr,time_ptr);
		...
		cout << "color flag is " << colorReadyFlag << endl;
		cout << "depth flag is " << depthReadyFlag << endl; 
		return false;
	}
}

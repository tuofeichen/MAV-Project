...
void distanceCallback(const std_msgs::Float64 dist);
void hover();
...
int main(int argc, char **argv) 
{
	...
	ros::Subscriber distance_sub = nh.subscribe("/DistanceToWall", 100, &distanceCallback);	
	...
	// second stage - find perpendicular wall
		...
		if(stages[1] && !stages[2])
		{
			...
			if(fabs(pos_read_pz - height) < 0.2)
			{
				stages[2] = true;
				hover();
				move_pub.publish(MovePosition);
		...
		// fourth stage - go to wall
		if(stages[2] && !stages[3])
		{
			
			ROS_INFO("Stage 4 - Going towards wall");

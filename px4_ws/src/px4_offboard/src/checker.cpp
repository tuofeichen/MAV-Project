...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    if (is_update) {
      ...
      ROS_INFO("Checker Publishing setpoint: x %f  y %f yaw %f", g_command.position.x,
...
void checkerCallback(const geometry_msgs::PoseStamped checker_pose) {
  ...
  double angular = 0.5;
  is_update = 1;
  if (checker_pose.pose.orientation.x > 0.1)
    g_command.position.y = linear;
  else if (checker_pose.pose.orientation.x < -0.1)
    g_command.position.y = -linear;

  if (checker_pose.pose.orientation.y > 0.1)
    g_command.position.x = -linear;
  else if (checker_pose.pose.orientation.y < -0.1)
    g_command.position.x = linear;

  if (checker_pose.pose.orientation.z > 0.1)
    g_command.yaw = -angular;
  else if (checker_pose.pose.orientation.z < -0.1)
    g_command.yaw = angular;
}
